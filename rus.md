...
с командой `node` у вас появилась команда `npm`. Этот инструмент даёт вам
...
существующие модули перед тем, как изобретать велосипед. Для приложения на 
Node.js нормально иметь массу зависимостей.
...
Давайте проведем рефакторинг скрипта, обрабатывающего логи. Его будет легче
...
Файлы подключаются точно так же, как и модули, за исключением того, что
